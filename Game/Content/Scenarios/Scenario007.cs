...
public class Scenario007 : ScenarioModel
{
	...
	private async GDTask SearchWater(Character character, Water water, int tokenNumber)
	{
		if(_waterTiles.Count % 4 == 0)
		{
			...
			switch(_waterTiles.Count)
			{
				case 8: // 9J
				{
					...
					{
						await AbilityCmd.SufferDamage(character, HazardousTerrain.DamageAmount, this);
					}
				...
				case 4: // 9E
				{
					...
					else
					{
						ActionState actionState =
							new ActionState(character, [HealAbility.Builder().WithHealValue(3).WithTarget(Target.Self)]);
						await actionState.Perform();
		...
		switch(tokenNumber)
		{
			...
			case 7: // 9A
			{
				ActionState actionState = new ActionState(character, [HealAbility.Builder().WithHealValue(2).WithTarget(Target.Self)]);
				await actionState.Perform();
			...
			case 11: // 9M
			{
				...
				{
					await AbilityCmd.SufferDamage(otherCharacter, 2, this);

					foreach(Summon summon in otherCharacter.Summons)
					{
						await AbilityCmd.SufferDamage(summon, 2, this);
					}
				}
