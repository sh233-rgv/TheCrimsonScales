...
/// <summary>
/// An <see cref="Ability{T}"/> that makes an ability trigger on a certain initiative.
/// </summary>
public class TriggeredAbility : ActiveAbility<TriggeredAbility.State>
{
	...
	/// <summary>
	/// A convenience method that returns an instance of TriggeredBuilder.
	/// </summary>
	/// <returns></returns>
	...
	protected override async GDTask Activate(State abilityState)
	{
		...
		ScenarioEvents.NextActiveFigureEvent.Subscribe(abilityState, this,
			...
			async _ =>
			{
				...
				ScenarioEvents.RoundEndedEvent.Unsubscribe(abilityState, this);
			});

		ScenarioEvents.RoundEndedEvent.Subscribe(abilityState, this,
			_ => true,
			async _ =>
			{
				await new ActionState(abilityState.ActionState, abilityState.Performer, Abilities).Perform();
